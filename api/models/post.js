...
class Post {
  ...
  static async getByRecipient(recipient_id) {
    console.log(recipient_id);
    let response = await db.query(
      'SELECT * FROM post JOIN user_account ON post.recipient_id = user_account.user_id WHERE username = $1',
      [recipient_id]
    );
    console.log('Here is the response from SQL:', response.rows);
  ...
  static async create(data) {
    ...
    let response = await db.query(
      'INSERT INTO post (title, content, sender_id, recipient_id) VALUES ($1, $2, $3, $4) RETURNING post_id;',
